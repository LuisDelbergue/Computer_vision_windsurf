...
while True:    #while the video is playing
  ...
  ret,filtered = cv2.threshold(img,26,238,cv2.THRESH_BINARY)
  results = pose.process(filtered) #obtain the pose
  ...
  if results.pose_landmarks:  #if it finds the points
    for id, lm in enumerate(results.pose_landmarks.landmark): #obtain the data
        mpDraw.draw_landmarks(filtered, results.pose_landmarks, mpPose.POSE_CONNECTIONS) #draw them
  ...
  cv2.putText(filtered, str(int(fps)), (70, 50), cv2.FONT_HERSHEY_PLAIN, 3,(255, 0, 0), 3) #write it in the window

  #saving.write(img)

  cv2.imshow("video", filtered)  #show the video
